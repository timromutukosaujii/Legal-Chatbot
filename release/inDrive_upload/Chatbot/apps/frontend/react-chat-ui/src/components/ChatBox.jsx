...
const disclaimer = "This chatbot provides general legal information, not legal advice.";
...
export default function ChatBox({ token, conversation, onConversationChange, onUsageUpdate, notice, setNotice }) {
  ...
  return (
    <section className="chat-card">
      ...
      <p className="official-hint">{disclaimer}</p>
      ...
      <div className="suggestions">
        {suggestions.map((item) => (
          <button key={item} type="button" className="suggestion" onClick={() => setInput(item)}>
            {item}
          </button>
        ))}
      </div>
